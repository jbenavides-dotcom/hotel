      ...
      <div className="relative z-10 w-full max-w-7xl px-4 sm:px-6 pt-32 sm:pt-24 pb-20">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-5 py-2 rounded-full bg-brand-pink text-white text-[9px] sm:text-[10px] uppercase tracking-[0.3em] sm:tracking-[0.4em] font-bold mb-6 sm:mb-8 animate-fade-in">
            <CoffeeIcon className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> {t('hero.badge')}
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif mb-4 sm:mb-6 leading-[1.15] text-white">
            {t('hero.title.pre')} <br className="hidden sm:block" /><span className="italic text-brand-beige">{t('hero.title.highlight')}</span>
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
            ...
            <div className="flex flex-col gap-2 text-white/90 text-xs sm:text-sm">
              <div className="flex items-center gap-2">
                ...
                <span>{t('hero.schedule')} {COFFEE_TOUR_INFO.time}</span>
              </div>
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-brand-pink" />
                <span>{t('hero.from')} {COFFEE_TOUR_INFO.departurePoint}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <a
              ...
              className="bg-brand-pink text-white px-6 sm:px-8 py-4 sm:py-5 font-bold uppercase text-xs tracking-widest flex items-center justify-center gap-3 hover:scale-105 hover:bg-brand-pink/80 transition-all shadow-2xl rounded-full active:scale-95"
            ...
            <div className="flex items-center justify-center sm:justify-start gap-3 sm:gap-4 text-white/80 px-4 sm:px-6 py-3 sm:py-4 glass-morphism rounded-xl">
              ...
              <span className="text-xs uppercase tracking-widest font-medium">{t('hero.location.chip')}</span>
            </div>
          </div>
        </div>
      </div>
